...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/setup-admin', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const userId = req.user.id || (req.user.claims && req.user.id);
  ...
  // Course Discussion Endpoints with Threading Support
  
  // Get discussions for a course with threading
  app.get("/api/courses/:courseId/discussions", async (req: Request, res: Response) => {
    try {
      const { courseId } = req.params;
      const { limit = 20, offset = 0, sortBy = "recent" } = req.query;
      
      // Get top-level discussions (parentId is null)
      const discussions = await db
        .select({
          ...
          contentType: courseDiscussions.contentType,
          isSticky: courseDiscussions.isSticky,
          isLocked: courseDiscussions.isLocked,
          isResolved: courseDiscussions.isResolved,
          isAnnouncement: courseDiscussions.isAnnouncement,
          likes: courseDiscussions.likes,
          dislikes: courseDiscussions.dislikes,
          replyCount: courseDiscussions.replyCount,
          viewCount: courseDiscussions.viewCount,
          createdAt: courseDiscussions.createdAt,
          updatedAt: courseDiscussions.updatedAt,
          lastActivityAt: courseDiscussions.lastActivityAt,
          // User info
        ...
        .where(and(
          eq(courseDiscussions.courseId, parseInt(courseId)),
          isNull(courseDiscussions.parentId)
        ))
        .orderBy(
          sortBy === "popular" ? desc(courseDiscussions.likes) :
          sortBy === "oldest" ? asc(courseDiscussions.createdAt) :
          desc(courseDiscussions.lastActivityAt)
        )
        .limit(parseInt(limit as string))
        .offset(parseInt(offset as string));

      // Get reply counts and recent replies for each discussion
      const discussionIds = discussions.map(d => d.id);
      const replies = discussionIds.length > 0 ? await db
        .select({
          discussionId: courseDiscussions.parentId,
          id: courseDiscussions.id,
          userId: courseDiscussions.userId,
          content: courseDiscussions.content,
          likes: courseDiscussions.likes,
        ...
        .from(courseDiscussions)
        .leftJoin(users, eq(courseDiscussions.userId, users.id))
        .where(inArray(courseDiscussions.parentId, discussionIds))
        .orderBy(desc(courseDiscussions.createdAt))
        .limit(100) : [];

      // Group replies by discussion
      const repliesByDiscussion = replies.reduce((acc, reply) => {
        if (!acc[reply.discussionId!]) acc[reply.discussionId!] = [];
        acc[reply.discussionId!].push(reply);
        return acc;
      }, {} as Record<number, typeof replies>);

      // Attach recent replies to discussions
      const discussionsWithReplies = discussions.map(discussion => ({
        ...discussion,
        replies: repliesByDiscussion[discussion.id]?.slice(0, 3) || [],
        hasMoreReplies: (repliesByDiscussion[discussion.id]?.length || 0) > 3
      }));

      res.json({
        discussions: discussionsWithReplies,
        hasMore: discussions.length === parseInt(limit as string)
      });
    } catch (error) {
      console.error("Error fetching course discussions:", error);
  ...
  // Create new discussion or reply
  app.post("/api/courses/:courseId/discussions", isAuthenticated, async (req: any, res: Response) => {
    try {
      const { courseId } = req.params;
      const { title, content, parentId, contentType = "markdown" } = req.body;
      const userId = req.user.id;
      ...
      if (!parentId && !title?.trim()) {
        return res.status(400).json({ error: "Title is required for new discussions" });
      }

      // Verify course enrollment for students
      if (req.user.role === 'student') {
        ...
        if (enrollment.length === 0) {
          return res.status(403).json({ error: "Must be enrolled to participate in discussions" });
        }
      }
      ...
      // Update parent discussion's reply count and last activity if this is a reply
      if (parentId) {
        await db
          ...
          .where(eq(courseDiscussions.id, parseInt(parentId)));
      }
  ...
  app.get("/api/discussions/:discussionId/replies", async (req: Request, res: Response) => {
    try {
      ...
      const replies = await db
        ...
        .from(courseDiscussions)
        .leftJoin(users, eq(courseDiscussions.userId, users.id))
        .where(eq(courseDiscussions.parentId, parseInt(discussionId)))
        .orderBy(asc(courseDiscussions.createdAt))
        .limit(parseInt(limit as string))
        .offset(parseInt(offset as string));
  ...
  app.post("/api/discussions/:discussionId/react", isAuthenticated, async (req: any, res: Response) => {
    try {
      const { discussionId } = req.params;
      ...
      const existingReaction = await db
        ...
        .where(and(
          eq(discussionReactions.discussionId, parseInt(discussionId)),
      ...
      if (existingReaction.length > 0) {
        ...
        await db
          ...
          .where(eq(discussionReactions.id, existingReaction[0].id));
        ...
        await db
          ...
          .set({
            [reactionType === 'like' ? 'likes' : 'dislikes']: 
              sql`${reactionType === 'like' ? courseDiscussions.likes : courseDiscussions.dislikes} - 1`
          })
      ...
      res.status(500).json({ error: "Failed to react to discussion" });
    }
  });
